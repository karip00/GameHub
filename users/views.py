...
from .forms import UserRegistrationForm, UserLoginForm, UserUpdateForm, PasswordResetForm, SetPasswordForm
            ...
            if associated_user:
                ...
                if email.send():
                    messages.success(request,
                        """
                        <h2>Password reset email sent</h2><hr>
                        <p>
                            ...
                            you registered with, and please check your spam folder.
                        </p>
                        """
                    )
                else:
...
def activateEmail(request, user, to_email):
    ...
    if email.send():
        ...
        messages.error(request, f'Problem sending confirmation email to {to_email}, check if it was typed correctly.')
...
def subscribe(request):
    if request.method == 'POST':
        ...
        if not name or not email:
            messages.error(request, "You must type a legitimate name and email to subscribe to the Newsletter")
            return redirect("/")
